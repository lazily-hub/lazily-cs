        ...
        foreach (var scenario in SpecCorpus.Scenarios(doc.RootElement, Corpus, Fixture).All())
        {
            ...
            if (scenario.TryGetProperty("blocks", out var blocksEl))
            {
                ...
                expect.TryAssertKeyWith(
                    "key_equal",
                    equalPairs => equalPairs.Against(keys, (expect, got) =>
                    {
                        foreach (var pair in expect.EnumerateArray())
                        {
                            var (i, j) = (pair[0].GetInt32(), pair[1].GetInt32());
                            Check($"key_equal[{i},{j}]", got[i] == got[j], true);
                        }
                    }));

                expect.TryAssertKeyWith(
                    "key_not_equal",
                    notEqualPairs => notEqualPairs.Against(keys, (expect, got) =>
                    {
                        foreach (var pair in expect.EnumerateArray())
                        {
                            var (i, j) = (pair[0].GetInt32(), pair[1].GetInt32());
                            Check($"key_not_equal[{i},{j}]", got[i] == got[j], false);
                        }
                    }));
            ...
            if (expect.TryGetProperty("matches", out _))
            {
                ...
                expect.AssertKeyWith(
                    "matches",
                    wantMatches => wantMatches.Against(
                ...
                expect.AssertKeyWith(
                    "removed",
                    want => want.Against(string.Join(",", alignment.Removed), (expect, got) => Check(
                        "removed",
                        got,
                ...
                expect.TryAssertKeyWith(
                    "similarity_min",
                    simMin => simMin.Against(
                        alignment.NewMatches.Where(m => m.Kind is MatchKind.Edited).ToList(),
                        (expect, edited) =>
                        {
                            Check("similarity_min.any_edited", edited.Count > 0, true);
                            Check(
                                "similarity_min",
                                edited.All(m => m.Similarity >= expect.GetDouble()),
                                true);
                        }));
            }
            ...
            expect.TryAssertKeyWith(
                ...
                flowPairs => flowPairs.Against(
                    StableId.AssignStableKeys(oldBlocks, newBlocks),
                    (expect, assigned) =>
                    {
                        foreach (var pair in expect.EnumerateArray())
                        {
